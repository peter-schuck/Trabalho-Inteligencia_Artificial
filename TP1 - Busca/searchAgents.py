...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.goal = (1,1,1,1)
        self.distances = [
            [0,top-1,right-1,top+right-2],
            [top-1,0,top+right-2,right-1],
            [right-1,top+right-2,0,top-1],
            [top+right-2,right-1,top-1,0]
        ]
    ...
    def getStartState(self):
        ...
        for i, temp in enumerate(self.corners):
            if temp == (position):
                new_corners = corners[:i] + (1,) + corners[i+1:]
                corners = new_corners
        #all this to cover cases where pacman starting position is in the corner
...
def cornersHeuristic(state, problem):
    ...
    remaining = [corners[i] for i, visited in enumerate(corners_goal) if visited == 0]
...
def foodHeuristic(state, problem):
    ...
    #         dx = coordsa[0] - coordsb[0]
    #         dy = coordsa[1] - coordsb[1]
    #         if n < dy: n = dy
    #         if s > dy: s = dy
    #         if e < dx: e = dx
    #         if w > dx: w = dx

    # return (n - s + e - w) / (foodGrid.count() + 1)
